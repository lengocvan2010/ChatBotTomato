...
app = FastAPI()
def load_model(model_path="D:\VsCode\Project1\model\ckpt_epoch_13.weights.h5"):
...
async def upload_image(
    text: str = Form(...), 
    file: UploadFile = File(None)  # Đặt file là tùy chọn
):
    if not file:
        # Không có file, chỉ xử lý text
        result_webSearch = process_search_results(search_web(f"Thông tin và cách chăm sóc cây cà chua"))
        infoFromDB = retrival(text)

        prompt = f"Đóng vai trò là chuyên gia chăm sóc cây trồng, hãy dựa vào '{infoFromDB}' và '{result_webSearch}' sau đó trả lời câu hỏi '{text}' bằng tiếng Việt đúng trọng tâm. Nhớ thật kỹ là bằng tiếng Việt."

        result = genAns(prompt)
        ...
        try:
            ...
            result_webSearch = process_search_results(search_web(f"Thông tin và cách chăm sóc cây cà chua"))
            infoFromPredictions = read_file_from_folder(predictions + 'txt', "D:\\VsCode\\Project1\\doc")

            infoFromDB = retrival(f"tình trạng cây {predictions}" + text)

            prompt = f"Đóng vai trò là chuyên gia chăm sóc cây trồng, hãy dựa vào tình trạng của cây cà chua: '{predictions}', '{infoFromPredictions}', đọc thêm thông tin '{infoFromDB}' và '{result_webSearch}', sau đó trả lời câu hỏi '{text}' bằng tiếng Việt đúng trọng tâm. Nhớ thật kỹ là bằng tiếng Việt."
...
client = Groq(
    api_key="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",
)
